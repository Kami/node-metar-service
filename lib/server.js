...
Server.prototype._initialize = function(callback) {
  ...
  this._app.use(express.limit(MAX_BODY_SIZE));
...
Server.prototype._parseMetarValues = function(values) {
  var i, value, prefix, item, parsed, result;
  ...
  for (i = 0; i < values.length; i++) {
    ...
    // Remove optional 'METAR' prefix
    prefix = 'METAR ';
    if (underscore.isString(value) && value.indexOf(prefix) === 0) {
...
Server.prototype._handleMetarRequest = function(req, res) {
  var payload = req.body, value, isString, isArray, parsed, result;
  ...
  value = payload.value;
  isString = underscore.isString(value);
